  ...
  const getRanking = () => {
    ...
    .then(async (resp) => {
      ...
      for (let i = 0; i < ranking.length; i++) {
        getGithubStars(ranking[i].githubId, ranking.length)
      }

    })
  }

  const getGithubStars = (username: string, length: number) => {
      ...
      <div className={style.rankingContainer}>
        <div className={style.container}>
          <div className={style.menu}>
            ...
            { plusStatus ? <button className={style.menuBtn} onClick={addAccount}>내 정보 등록</button> : <button className={style.menuBtnDisable}>내 정보 등록</button> }
          </div>

          { ranking?.map((el: any, idx: number) => {
            return <div key={idx} className={style.graphContainer}>
              <img className={style.graphImg} src={`https://avatars.githubusercontent.com/${el.githubId}`} />
